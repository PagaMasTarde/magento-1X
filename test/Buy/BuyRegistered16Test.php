...
class BuyRegistered16Test extends AbstractBuy16
{
    ...
    {
        $notifyUrl = $this->magentoUrl16.self::NOTIFICATION_FOLDER.'?order=';
    ...
    {
        $orderId=0;
        $notifyUrl = $this->magentoUrl16.self::NOTIFICATION_FOLDER.'?order='.$orderId;
        $this->assertNotEmpty($notifyUrl, $notifyUrl);
        $response = Request::post($notifyUrl)->expects('json')->send();
        $this->assertNotEmpty($response->body->result, $response);
        $this->assertNotEmpty($response->body->status_code, $response);
    ...
    {
        $notifyUrl = $this->magentoUrl16.self::NOTIFICATION_FOLDER.'?order=145000008';
        $response = Request::post($notifyUrl)->expects('json')->send();
    ...
    {
        $configUrl = $this->magentoUrl16.self::CONFIG_FOLDER.'get?secret='.$this->configuration['secretKey'];
        ...
        $configUrl = $this->magentoUrl16.self::CONFIG_FOLDER.'post?secret='.$this->configuration['secretKey'];
