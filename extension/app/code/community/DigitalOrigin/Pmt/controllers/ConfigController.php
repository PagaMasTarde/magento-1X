 ...
 */
class DigitalOrigin_Pmt_ConfigController extends AbstractController
{
    /**
     * @var bool $error
     */
    ...
    public function getAction()
    {
        ...
        $results = Mage::helper('pmt/ExtraConfig')->getExtraConfig();
        $this->statusCode = 200;
    ...
    public function postAction()
    {
        ...
        $output = Mage::helper('pmt/ExtraConfig')->getExtraConfig();
        if (count($_POST)) {
            $post = $_POST;
            foreach ($output as $key => $value) {
                if (array_key_exists($key, $post)) {
                    $conn = Mage::getSingleton('core/resource')->getConnection('core_write');
                    $sql = 'update pmt_config set value = \''. $post[$key] .'\' 
                            where config = \'' . $key . '\'';
                    $conn->query($sql);
                    $value = $post[$key];
                    unset($post[$key]);
